 ...
 * ======================================================================== */
/** Source file for SoColorBlobTrakEngine engine
  *
...
SO_ENGINE_SOURCE(SoColorBlobTrakEngine);

void SoColorBlobTrakEngine::initClass(void)
{    
    SO_ENGINE_INIT_CLASS(SoColorBlobTrakEngine,SoEngine,"Engine");
}

SoColorBlobTrakEngine::SoColorBlobTrakEngine(void) : adapter(NULL)
{
    SO_ENGINE_CONSTRUCTOR(SoColorBlobTrakEngine);
...
SoColorBlobTrakEngine::~SoColorBlobTrakEngine()
{
    if (adapter!=NULL) delete adapter;
}


void SoColorBlobTrakEngine::inputChanged(SoField* whichField)
{
    if (whichField==&key || whichField==&value){
        delete adapter;
        adapter=new EventAdapter<SoColorBlobTrakEngine>(this);
    }
}

void SoColorBlobTrakEngine::processEvent(SoInputEvent *event)
{
    if (event){
