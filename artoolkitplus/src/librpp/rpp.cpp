 ...
 *
 * $Id: rpp.cpp 212 2007-05-09 13:11:53Z bornik $
 * @file
 * ======================================================================== */
...
#include <ARToolKitPlus/librpp/rpp.h>
#include <ARToolKitPlus/librpp/rpp_const.h>
#include <ARToolKitPlus/librpp/rpp_vecmat.h>
...
//  from all solutions get the 2nd Best one !!
  ...
  for(unsigned int i=0; i<sol.size(); i++)
    if(sol[i].obj_err < min_err  && 
       (min_err_idx2 != static_cast<int>(i)) )
    {
      min_err = sol[i].obj_err;
      min_err_idx = i;
    }
