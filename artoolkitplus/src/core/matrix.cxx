 ...
 *
 * $Id: matrix.cxx 212 2007-05-09 13:11:53Z bornik $
 * @file
 * ======================================================================== */
...
// ARFloat mdet(ARFloat *ap, int dimen, int rowa);
ARFloat *minv( ARFloat *ap, int dimen, int rowa );
...
selfInv(ARMat *m)
{
	int i,num = m->row*m->row;
	ARFloat* fArray = (ARFloat*)::malloc(num*sizeof(ARFloat));
	Matrix::FLOAT *mV = m->m;

	for(i=0; i<num; i++)
		fArray[i] = (ARFloat)mV[i];

	//if(minv(m->m, m->row, m->row) == NULL) return -1;
	if(minv(fArray, m->row, m->row) == NULL)
	{
		::free(fArray);
		return -1;
	}
...
#if 0
FLOAT
mdet(FLOAT *ap, int dimen, int rowa)
