 ...
 *
 * $Id: vector.cxx 212 2007-05-09 13:11:53Z bornik $
 * @file
 * ======================================================================== */
...
alloc( int clm )
{
    ...
    v->v = (FLOAT *)malloc(sizeof(FLOAT) * clm);
    if( v->v == NULL ) {
        free(v);
        return NULL;
    }
...
household( ARVec *x )
{
    ...
    s = (FLOAT)sqrt( innerproduct(x,x) );

    if( s != 0.0f ) {
        if(x->v[0] < 0.0f) s = -s;
        x->v[0] += s;
        t = (FLOAT)1.0f / (FLOAT)sqrt((x->v[0] * s));
        for( i = 0; i < x->clm; i++ ) {
            x->v[i] *= t;
        }
    }
